        ...
        handler: async (req, res) => {
            ...
            try {
                ...
                if (req.method === "POST" && method === "apply/proposals") {
                    ...
                    if (apply === undefined) {
                        writeJson(res, 503, { ok: false, error: { code: "SETTINGS_UNAVAILABLE", message: "Apply 未启用（settings seam 缺失）" } });
                        return;
                    }
                    ...
                    try {
                        ...
                        if (error instanceof ApplyError) {
                            writeJson(res, applyErrorStatus(error.code), { ok: false, error: { code: error.code, message: error.message } });
                            return;
                        }
                        throw error;
                    }
                ...
                if (req.method === "GET" && method.startsWith("apply/proposals/")) {
                    const apply = svc.apply;
                    if (apply === undefined) {
                        writeJson(res, 503, { ok: false, error: { code: "SETTINGS_UNAVAILABLE", message: "Apply 未启用" } });
                        return;
                    }
                    const id = method.slice("apply/proposals/".length);
                    const proposal = apply.getProposal(id);
                    if (proposal === undefined) {
                        writeJson(res, 404, { ok: false, error: { code: "INVALID_PROPOSAL", message: "提案不存在" } });
                        return;
                    }
                    writeJson(res, 200, { ok: true, proposal });
                    return;

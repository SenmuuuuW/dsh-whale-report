 ...
 * 宽松正则解析，取不带引号的 key 值。
 */
export function readCredentialsKey(filePath, name) {
    if (!existsSync(filePath))
        return null;
    try {
        const raw = readFileSync(filePath, "utf8");
...
export function parseDeepSeekBalance(json) {
    ...
    const parsed = infos.map(pick).filter((p) => p !== null);
    if (parsed.length === 0)
        return null;
    const info = parsed.find((p) => p.currency === "CNY") ??
...
const STABLE_STATUSES = ["connected", "invalid-key", "unavailable"];

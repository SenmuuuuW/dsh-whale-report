...
function TrendChart({ label, values, color, unit, keys, liveLast, }) {
    const W = 220;
    const H = 64;
    const P = 4;
    if (values.length < 2)
    ...
    return (_jsxs("div", { "data-whale-report-trendchart": true, children: [_jsxs("div", { "data-whale-report-trendhead": true, children: [_jsx("b", { children: label }), _jsxs("span", { children: [fmt(last), unit, liveLast === true && _jsx("em", { "data-whale-report-trendlive": true, children: "LIVE" })] })] }), _jsxs("svg", { viewBox: `0 0 ${W} ${H}`, width: "100%", height: H, role: "img", "aria-label": `${label} 趋势`, children: [_jsx("line", { x1: P, y1: H - P, x2: W - P, y2: H - P, stroke: "var(--dt-line)", strokeWidth: "1" }), pts.map(([x, y], i) => (_jsxs("g", { children: [i > 0 && (_jsx("line", { x1: pts[i - 1][0], y1: pts[i - 1][1], x2: x, y2: y, stroke: color, strokeWidth: "1.8", strokeDasharray: liveLast === true && i === lastIdx ? "3 3" : undefined })), liveLast === true && i === lastIdx ? (_jsx("circle", { cx: x, cy: y, r: "2.6", fill: "var(--dt-paper-deep)", stroke: color, strokeWidth: "1.4" })) : (_jsx("circle", { cx: x, cy: y, r: "2.4", fill: color }))] }, i)))] }), _jsxs("div", { "data-whale-report-trendkeys": true, children: [_jsx("span", { children: keys[0] }), _jsx("span", { children: liveLast === true ? `${keys[keys.length - 1]} · LIVE` : keys[keys.length - 1] })] })] }));
}
function TrendSection({ preset }) {
    ...
    if (trends === null || trends.length < 2)
        return null;
    ...
    const chart = (label, values, color, unit) => (_jsx(TrendChart, { label: label, values: values, color: color, unit: unit, keys: keys, liveLast: liveLast }));
    return (_jsxs("section", { "data-whale-report-section": true, children: [_jsx(SectionHeader, { index: "\u2014", title: "\u5386\u53F2\u8D8B\u52BF", meta: `HISTORY / ${completeCount} COMPLETE${liveCount > 0 ? ` · ${liveCount} LIVE` : ""}` }), _jsxs("div", { "data-whale-report-trendgrid": true, children: [chart("COST", trends.map((t) => t.cost), "var(--dt-blue)", ""), chart("SESSIONS", trends.map((t) => t.sessions), "var(--dt-cyan)", ""), chart("CACHE HIT", trends.map((t) => t.cacheHitRate), "var(--dt-safe, #31765a)", "%"), chart("NIGHT", trends.map((t) => t.nightRatio), "var(--dt-amber)", "%")] })] }));
}

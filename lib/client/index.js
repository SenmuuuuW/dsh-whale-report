...
export class WhaleContent extends Component {
    state = {
        toast: null,
        view: "dashboard",
        preset: "weekly",
    ...
    async pollOverview(preset) {
        try {
            ...
            const body = (await response.json());
            if (!response.ok || body.ok === false || body.snapshot !== true || body.report === undefined)
                return;
...
function PricePeriodCard() {
    ...
    const next = weekend
        ? (() => {
            ...
            return { m: 9 * 60, label: "周一 9:00", to: "peak", atMs: dayStartUtc + daysAhead * 86400000 + 9 * 3600_000 - 8 * 3600_000 };
        })()
        : null;
    const weekdayNext = next === null ? (PERIOD_BOUNDS.find((b) => b.m > cstClock) ?? { m: 33 * 60, label: "9:00", to: "peak" }) : next;
    ...
    return (_jsxs("div", { "data-whale-report-price": true, "data-period": period, children: [_jsxs("div", { "data-whale-report-pricehead": true, children: [_jsx("span", { "data-whale-report-pricecode": true, children: "CURRENT RATE \u00B7 \u00A5/1M TOKEN / \u5F53\u524D\u4EF7\u683C" }), _jsx("em", { "data-whale-report-priceperiod": true, children: peak ? "高峰 · PEAK" : "谷时 · OFF-PEAK" })] }), _jsx("div", { "data-whale-report-ratetable": true, children: RATE_ROWS.map((r) => {
                ...
                }) }), _jsxs("div", { "data-whale-report-pricestrip": true, role: "img", "aria-label": "\u5CF0\u8C37\u65F6\u6BB5\u5206\u5E03\uFF1A\u5DE5\u4F5C\u65E5 9\u201312 \u4E0E 14\u201318 \u65F6\u9AD8\u5CF0\uFF08\u7425\u73C0\u8272\uFF09\uFF0C\u5176\u4F59\u8C37\u65F6\uFF08\u9752\u8272\uFF09\uFF1B\u5468\u672B\u5168\u5929\u8C37\u65F6\uFF082026-08-23 \u8D77\uFF09\uFF1B\u6307\u9488\u6807\u793A\u5F53\u524D\u65F6\u523B", children: [hours.map((pk, h) => (_jsx("i", { "data-peak": pk, "data-now": h === cstHour }, h))), _jsx("div", { "data-whale-report-stripneedle": true, style: { left: `${((cstClock / (24 * 60)) * 100).toFixed(2)}%` } })] }), _jsxs("div", { "data-whale-report-stripticks": true, "aria-hidden": "true", children: [_jsx("span", { children: "0" }), _jsx("span", { children: "6" }), _jsx("span", { children: "12" }), _jsx("span", { children: "18" }), _jsx("span", { children: "24" })] }), _jsxs("div", { "data-whale-report-pricecount": true, children: [next === null
                        ? _jsxs(_Fragment, { children: ["\u8DDD ", weekdayNext.label, " \u8F6C", weekdayNext.to === "peak" ? "高峰" : "谷时", " ", countdown] })
                        : _jsxs(_Fragment, { children: ["\u5468\u672B\u5168\u5929\u8C37\u65F6 \u00B7 \u8DDD ", next.label, " \u8F6C\u9AD8\u5CF0 ", countdown] }), " \u00B7 \u5DE5\u4F5C\u65E5\u9AD8\u5CF0 9\u201312 / 14\u201318 \u5317\u4EAC\u65F6\u95F4", peak ? " · 谷时 5 折" : " · 高峰 2×"] }), noticed && (_jsx("div", { "data-whale-report-pricenotice": true, children: peak
...
function Dashboard(props) {
    ...
    const report = dashboard !== null && dashboard.preset === preset
...
function ApplyFlow({ item }) {
    ...
    const fmtMs = (ms) => (ms >= 60_000 ? `${Math.round(ms / 1000 / 60)}min` : `${Math.round(ms / 1000)}s`);
    if (phase === "idle") {
        return (_jsx("div", { "data-whale-report-applybtn": true, children: _jsx("button", { onClick: () => void review(), children: "Review change" }) }));
    }
    if (phase === "loading") {
        return (_jsx("div", { "data-whale-report-applybtn": true, children: _jsx("button", { disabled: true, children: "\u5904\u7406\u4E2D\u2026" }) }));
    }
    if (phase === "error") {
        return (_jsxs("div", { children: [_jsx("div", { "data-whale-report-applyerr": true, children: msg ?? "未知错误" }), _jsxs("div", { "data-whale-report-applybtn": true, children: [_jsx("button", { onClick: () => void review(), children: "\u91CD\u8BD5" }), _jsx("button", { onClick: cancel, children: "\u5173\u95ED" })] })] }));
    }
    if (phase === "ready" && proposal !== null) {
        const pct = (n) => `${(n * 100).toFixed(1)}%`;
        return (_jsxs("div", { "data-whale-report-apply": true, children: [_jsxs("div", { "data-whale-report-applyrow": true, children: [_jsx("span", { children: "PROBLEM" }), _jsx("p", { children: proposal.reason })] }), _jsxs("div", { "data-whale-report-applyrow": true, children: [_jsx("span", { children: "EVIDENCE" }), _jsxs("p", { children: [proposal.evidence.timeoutCount, " \u6B21\u786E\u5B9A\u6027 timeout \u00B7 ", proposal.evidence.timeoutSessions.length, " \u4E2A\u4F1A\u8BDD \u00B7", " ", proposal.evidence.shellInvocationCount, " \u6B21 shell \u8C03\u7528"] })] }), _jsxs("div", { "data-whale-report-applyrow": true, children: [_jsx("span", { children: "CHANGE" }), _jsxs("p", { children: ["shell.timeoutMs ", _jsx("b", { children: fmtMs(proposal.expectedBefore) }), " \u2192 ", _jsx("b", { children: fmtMs(proposal.proposedAfter) })] })] }), _jsxs("div", { "data-whale-report-applyrow": true, children: [_jsx("span", { children: "EXPECTED" }), _jsxs("p", { children: ["shell_timeout_rate \u2264 ", pct(proposal.verificationPlan.target.value), "\uFF08\u6837\u672C \u2265", proposal.verificationPlan.minimumEvidence.observations, " \u6B21\u8C03\u7528 / \u2265", proposal.verificationPlan.minimumEvidence.sessions, " \u4F1A\u8BDD\uFF09"] })] }), _jsxs("div", { "data-whale-report-applyrow": true, children: [_jsx("span", { children: "ROLLBACK" }), _jsxs("p", { children: ["\u4E00\u952E\u8FD8\u539F\u4E3A ", _jsx("b", { children: fmtMs(proposal.rollbackPlan.value) }), "\uFF08\u5E76\u53D1\u5B89\u5168\uFF09"] })] }), _jsxs("div", { "data-whale-report-applybtn": true, children: [_jsx("button", { "data-primary": true, onClick: () => void apply(), children: "Apply" }), _jsx("button", { onClick: () => void cancel(), children: "Cancel" })] })] }));
    }
    // applied / 终态
    const vStatus = verify?.status ?? "observing";
    const chip = vStatus === "reverted"
        ? "REVERTED"
        : vStatus === "verified"
            ? "VERIFIED"
            : vStatus === "not_improved"
                ...
                : vStatus === "inconclusive"
                    ? "INCONCLUSIVE"
                    : "OBSERVING";
    return (_jsxs("div", { "data-whale-report-apply": true, children: [_jsxs("div", { "data-whale-report-applyrow": true, children: [_jsx("span", { children: "STATUS" }), _jsxs("p", { children: [_jsx("span", { "data-whale-report-applychip": true, "data-status": vStatus, children: chip }), verify !== null && verify.record.observed !== null && (_jsxs(_Fragment, { children: [" \u00B7 shell_timeout_rate ", (verify.record.observed.value ?? 0) * 100 > 0 ? `${((verify.record.observed.value ?? 0) * 100).toFixed(2)}%` : "—", " \u00B7 \u6837\u672C ", verify.progress.observations, "/", verify.record.minimumEvidence?.observations ?? "?"] }))] })] }), verify?.verdictNote !== undefined && (_jsxs("div", { "data-whale-report-applyrow": true, children: [_jsx("span", { children: "VERIFY" }), _jsx("p", { children: verify.verdictNote })] })), verify?.record.observed !== null && (_jsxs("div", { "data-whale-report-applyrow": true, children: [_jsx("span", { children: "OBSERVED" }), _jsxs("p", { children: ["\u57FA\u7EBF ", (verify?.record.baseline?.value ?? 0) * 100 > 0 ? `${((verify?.record.baseline?.value ?? 0) * 100).toFixed(2)}%` : "—", " \u2192 \u89C2\u5BDF ", (verify?.record.observed?.value ?? 0) * 100 > 0 ? `${((verify?.record.observed?.value ?? 0) * 100).toFixed(2)}%` : "—", "\uFF08\u76EE\u6807 \u2264", ((verify?.record.targetValue ?? 0) * 100).toFixed(2), "%\uFF09"] })] })), (vStatus === "not_improved" || vStatus === "inconclusive") && (_jsx("div", { "data-whale-report-applybtn": true, children: _jsx("button", { "data-danger": true, onClick: () => void revert(), children: "Revert\uFF08\u6062\u590D\u539F\u503C\uFF09" }) }))] }));
}

...
function printableInsights(value) {
    ...
    for (const item of value) {
        if (typeof item !== "object" || item === null)
            continue;
        const row = item;
        if (typeof row.title !== "string" || typeof row.detail !== "string" || typeof row.action !== "string")
            continue;
...
function previousCost(record) {
    if (typeof record.prev !== "object" || record.prev === null)
        return null;
    const cost = record.prev.cost;
...
function findingsHtml(record) {
    ...
    const levelLabel = (level) => level === "critical" ? "CRITICAL" : level === "warning" ? "WATCH" : level === "tip" ? "NOTE" : "INFO";
    return insights
        .map((insight, index) => `
...
function printableImprovements(value) {
    if (!Array.isArray(value))
        return [];
    const items = [];
    for (const raw of value) {
        ...
        const plan = (item.verificationPlan ?? {});
        ...
        items.push({
            ...
            evidence: {
                ...
                affectedSessions: Array.isArray(evidence.affectedSessions) ? evidence.affectedSessions.filter((s) => typeof s === "string") : [],
...
function gridHtml(stats) {
    ...
    if (series.length === 0)
        return `<p class="empty-line">该报告生成于旧版本，无逐时数据。重新生成即可。</p>`;
    const max = Math.max(1, ...series.flatMap((item) => item.hours));
    const cell = (count) => {
        if (count === 0)
            return '<i class="scan-cell scan-cell--empty"></i>';
...
function tokenBarHtml(stats) {
    ...
    if (total === 0)
        return `<p class="empty-line">无 Token 用量数据。</p>`;
    const item = (label, sub, value, tone) => `
...
function modelTableHtml(stats, cost) {
    ...
    if (entries.length === 0)
        return `<p class="empty-line">无模型用量数据。</p>`;
    return `
...
function toolHealthHtml(stats) {
    ...
    const rows = health
        .filter((t) => t.calls >= 5)
        .sort((a, b) => {
        const ab = (x) => x.calls >= TOOL_HEALTH_MIN_CALLS && x.failed >= TOOL_HEALTH_MIN_FAILED && x.failureRate >= TOOL_HEALTH_MIN_FAILURE_RATE;
        if (ab(a) !== ab(b))
            return ab(a) ? -1 : 1;
        if (ab(a) && ab(b))
...
function sessionTokenTotal(session) {
    return Object.values(session.modelTokens ?? {}).reduce((total, usage) => total + usage.input + usage.output + usage.cacheRead + usage.reasoning, 0);
}
function collabHtml(record) {
...
function traceLogHtml(stats, cost) {
...
export function renderHtmlReport(record) {
    ...
    stats.hourHistogram = stats.hourHistogram ?? new Array(24).fill(0);
...
<main class="page">
  ...
  <footer class="report-foot">
    ...
    <span>${(() => {
        const gen = record.reportGeneration;

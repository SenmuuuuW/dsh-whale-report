...
function toolImprovement(h, period, now, failedSessions) {
    if (h.calls < IMPROVE_TOOL_MIN_CALLS)
        return null;
    if (h.failed < IMPROVE_TOOL_MIN_FAILED)
        return null;
    if (h.failureRate < IMPROVE_TOOL_MIN_FAILURE_RATE)
        return null;
    const sessions = failedSessions[h.name] ?? [];
    const sessionCount = sessions.length;
    if (sessionCount < IMPROVE_TOOL_MIN_SESSIONS)
        return null;
    ...
    if (mainCount < IMPROVE_MAIN_CODE_MIN || mainCount / h.failed < IMPROVE_MAIN_CODE_SHARE)
        return null;

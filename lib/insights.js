...
export function toolHealthAttention(t) {
    return t.failed * (1 + t.failureRate);
}
/** 第 9 条确定性洞察：最值得关注的一个工具（最多一条，避免噪音）。 */
export function toolHealthInsight(health) {
    if (health === undefined || health.length === 0)
        return null;
    const candidates = health.filter((t) => t.calls >= TOOL_HEALTH_MIN_CALLS && t.failed >= TOOL_HEALTH_MIN_FAILED && t.failureRate >= TOOL_HEALTH_MIN_FAILURE_RATE);
...
export function shanghaiDayStart(ms) {
...
export function shanghaiDateKey(ms) {
 ...
 */
export function isTrendRowIncluded(key, recordPreset, prefix) {
    if (prefix === "")
        return true;
...
export function toolFamilies(toolCalls) {

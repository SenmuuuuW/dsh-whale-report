 ...
 */
import { aggregateBuckets, emptyPartial } from "./stats.js";
import { INDEX_VERSION } from "./tools.js";
/** 从 canonical index 查询一个精确周期。零会话 IO。 */
export function queryPeriod(index, spec, headers, meta) {
    const views = [];
    ...
    for (const h of headers) {
        ...
        views.push({ sessionId: entry.sessionId, buckets: entry.buckets, titles: entry.titles });

...
import { shanghaiDayStart, toolFamilies } from "./insights.js";
...
export function renderReport(stats, preset, cost, prev, insights, improvements) {
    ...
    if (stats.partial.skippedCount > 0 || stats.partial.salvage !== undefined) {
        ...
        if (p.salvage !== undefined)
            parts.push(`已通过只读恢复 ${salvageLine(p.salvage)}`);
        const missing = p.skippedCount > 0 || (p.salvage?.droppedRecords ?? 0) > 0;
        lines.push(`> ⚠️ DATA PARTIAL：${parts.join("；")}。` +
    ...
    if (improvements !== undefined && improvements.length > 0) {
        ...
        for (const item of improvements.slice(0, 3)) {
            ...
            lines.push(`  验证：${item.verificationPlan.targetMetric} 基线 ${item.verificationPlan.baseline ?? "—"} → 目标 ${item.verificationPlan.target}（${item.verificationPlan.window}）`);
            lines.push("");
        }
    }
    // —— 工作量
    ...
    if (cost !== undefined && cost.total > 0) {
        lines.push(`- 预估费用约 **¥${cost.total.toFixed(2)}**（${cost.source === "official-page" ? "官方定价页实时价" : cost.source === "peak-offpeak" ? "官方峰谷价（按时段）" : "内置价"}）`);
        const byProvider = {};

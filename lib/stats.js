 ...
 */
import { classifyCorrectionText } from "./improvements.js";
...
function newBucketToolHealthAcc(name) {
...
function textBlocksOf(content) {
    ...
    for (const block of content) {
        ...
        if (typeof nested === "string")
            ...
            texts.push(...textBlocksOf(nested));
    }
    return texts;
}
export function isTimeoutResult(data) {
    if (typeof data !== "object" || data === null)
        return false;
    if (errorCodeOf(data) === "TOOL_TIMEOUT")
        return true;
    const msg = data.message;
    if (typeof msg !== "object" || msg === null)
        return false;
    return textBlocksOf(msg.content).some((t) => TIMED_OUT_MARKER_RE.test(t));
}
...
export function aggregate(events, period, headers = []) {
        ...
        switch (event.type) {
                ...
                for (const text of textOfUserMessage(data)) {
                    ...
                    if (uagg.userMessages > 1) {
                        for (const category of classifyCorrectionText(text)) {
                            ...
                            if (agg.sampleIds.length < 8 && !agg.sampleIds.includes(sessionId))
                                agg.sampleIds.push(sessionId);
                        }
                    }
                }
                break;
            ...
            case "tool/call": {
                ...
                stats.toolCalls[name] = (stats.toolCalls[name] ?? 0) + 1;
                {
                    ...
                    if (sess === undefined) {
                        sess = [];
                        stats.toolInvocationSessions[name] = sess;
                    }
                    if (sess.length < 32 && !sess.includes(sessionId))
                        sess.push(sessionId);
                }
                {
                    const hkey = `${shanghaiDateKey(event.time)}T${String(shanghaiHour(event.time)).padStart(2, "0")}`;
    ...
    for (const [name, acc] of [...toolHealthAcc.entries()].sort((a, b) => (a[0] < b[0] ? -1 : 1))) {
        if (acc.failedSessions.size > 0)
            failedMap[name] = [...acc.failedSessions].slice(0, 100);
    }
 ...
 */
export function createOwnEventBucketizer(sessionId, ownStart, stopAfter) {
    ...
    const apply = (event) => {
        ...
        if (event.seq !== undefined && event.seq < ownStart)
            return true;
            ...
            case "tool/result": {
                ...
                // 工具健康：按 callId 配对（跨桶由会话级 pending 完成）。
                {
                    ...
                    if (pair !== undefined) {
                        ...
                        if (failed) {
                            acc.failed += 1;
                            if (!acc.failedSessions.includes(sessionId))
                                acc.failedSessions.push(sessionId);
                            const code = errorCodeOf(data);
                            acc.errorCodes[code] = (acc.errorCodes[code] ?? 0) + 1;
                        }
                        const duration = event.time - pair.time;
...
export function aggregateBuckets(views, period, headers = [], partial = emptyPartial()) {
    const stats = emptyStats(period);
    ...
    for (const view of views) {
        ...
        const applyEdgeBucket = (bucket, sessionId) => {
            let touched = false;
            const aggCur = agg;
            ...
            for (const row of bucket.rows ?? []) {
                if (row.t < period.from || row.t >= period.to)
                    continue;
                touched = true;
                if (row.k === 0) {
                    aggCur.turns += 1;
                    ...
                    if (row.n !== undefined) {
                        ...
                        if (sess === undefined) {
                            sess = [];
                            stats.toolInvocationSessions[row.n] = sess;
                        }
                        if (sess.length < 32 && !sess.includes(sessionId))
                            sess.push(sessionId);
                    }
                    ...
                    if (row.n !== undefined) {
                        ...
                        if (sess === undefined) {
                            sess = [];
                            stats.toolTimeoutSessions[row.n] = sess;
                        }
                        if (sess.length < 32 && !sess.includes(sessionId))
                            sess.push(sessionId);
                    }
        ...
        for (const bucket of view.buckets) {
            ...
            if (!inFrom || !inTo) {
                ...
                if (applyEdgeBucket(bucket, view.sessionId))
                    inWindow = true;
                continue;
            }
            inWindow = true;
            ...
            for (const [name, acc] of Object.entries(bucket.toolHealth ?? {})) {
                ...
                const failed = acc.failedSessions;
                if (failed instanceof Set) {
                    for (const sid of failed)
                        target.failedSessions.add(sid);
                }
            ...
            for (const [tool, sess] of Object.entries(bucket.toolTimeoutSessions ?? {})) {
                ...
                if (target === undefined) {
                    target = [];
                    stats.toolTimeoutSessions[tool] = target;
                }
                for (const sid of sess) {
                    if (target.length < 32 && !target.includes(sid))
                        target.push(sid);
                }
            }
            for (const [tool, sess] of Object.entries(bucket.toolInvocationSessions ?? {})) {
                ...
                if (target === undefined) {
                    target = [];
                    stats.toolInvocationSessions[tool] = target;
                }
                for (const sid of sess) {
                    if (target.length < 32 && !target.includes(sid))
                        target.push(sid);
            ...
            for (const c of bucket.corrections ?? []) {
                ...
                if (agg.sampleIds.length < 8 && !agg.sampleIds.includes(c.sessionId))
                    agg.sampleIds.push(c.sessionId);
            }

 ...
 */
const OPENCODE_ALIASES = new Set(["deepseek-modlens", ...(process.env.WHALE_PROVIDER_ALIASES ?? "").split(",").map((s) => s.trim()).filter(Boolean)]);
/** 从 request/header 事件里尽量识别 provider；识别不到返回 unknown。 */
function providerOf(data) {
    if (typeof data !== "object" || data === null)
        return "unknown";
    const d = data;
    ...
    if (typeof direct === "string" && direct !== "") {
        ...
        if (OPENCODE_ALIASES.has(direct))
            return "opencode-go";
        return direct;
    }

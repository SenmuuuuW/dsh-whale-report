...
import { computeCost, getPrices, modelCost, modelTier, OPENCODE_GO_PRICES } from "./pricing.js";
...
function whaleReportTool(svc) {
    return defineTool({
        ...
        execute: async (args, exec) => {
            ...
            // 不再把 whale/report 写进会话日志：核心 harness 不认插件自定义事件，
            // 且 Session.append 不支持 ignorable 标记，写入会导致会话历史加载失败。
            // 报告数据已由 periodStats 持久化。
            return {
                preset,

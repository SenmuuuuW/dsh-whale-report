...
import { computeCost, computeCostTimed, modelCost, modelTier, OPENCODE_GO_PRICES, OFFPEAK_PRICES, priceSetForTime } from "./pricing.js";
...
export async function buildReportFromStats(svc, preset, from, to, stats, perf) {
    ...
    if (stats.dayHourDetail.length > 0) {
        const timed = computeCostTimed(stats.dayHourDetail.flatMap((day) => day.hours.map((h, hour) => ({ time: shanghaiHourStartOf(day.date, hour), modelTokens: h.modelTokens }))));
    ...
    if (prevKey !== null) {
        ...
        if (cand !== null && cand.sem === REPORT_SEM)
            prev = cand;
    }

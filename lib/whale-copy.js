...
/** 收尾（按模式）。 */
export const NOTE_CLOSERS = {
    light: ["以上，就是本期小评。"],
    spicy: ["以上，仅供参考——反正你也不会听。"],
};
/** 无触发时的默认鲸评（数据干净）。 */
export const NOTE_CLEAN = [
    "“这期数据很干净呢，一点幺蛾子都没有。”",
    "（开心地晃了晃尾巴）这样的你，我特别喜欢。",
    "继续保持，我的任务就是让你省心呀。",
];
 ...
 */
export function buildWhaleNote(kinds, mood, mode = "light", n = 0) {
    const lines = [];
    for (const opener of NOTE_OPENERS[mood])
        lines.push({ text: opener, kind: "opener" });
    const top = kinds[0];
    if (top !== undefined) {
        ...
        for (const text of NOTE_CLEAN)
            lines.push({ text, kind: "body" });
    }
    for (const closer of NOTE_CLOSERS[mode])
        lines.push({ text: closer, kind: "closer" });
    return lines;
}
//# sourceMappingURL=whale-copy.js.map
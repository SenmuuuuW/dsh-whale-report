 ...
 */
/** 深夜占比（0-6 点事件百分比，整数）。 */
export function nightShare(input) {
 ...
 */
export function triggerNotes(input) {
    const hits = [];
    if ((input.dangerousCommands ?? []).some((d) => d.sev === "red"))
        hits.push({ kind: "danger", weight: 0 });
    else if (input.dangerousCommands.length > 0)
        hits.push({ kind: "danger", weight: 1 });
    if ((input.retryBursts ?? 0) >= 3)
        hits.push({ kind: "retry", weight: 2 });
    if (nightShare(input) >= 15)
 ...
 */
export function whaleMood(input) {
...
//# sourceMappingURL=whale-notes.js.map
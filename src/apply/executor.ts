...
export interface ApplyStore {
  proposalTable: { put(key: string, value: unknown): Promise<void> | void; get(key: string): unknown | undefined; entries?(): IterableIterator<[string, unknown]> | [string, unknown][] };
  recordTable: { put(key: string, value: unknown): Promise<void> | void; get(key: string): unknown | undefined; entries?(): IterableIterator<[string, unknown]> | [string, unknown][] };
  verifyTable: { put(key: string, value: unknown): Promise<void> | void; get(key: string): unknown | undefined; entries?(): IterableIterator<[string, unknown]> | [string, unknown][] };
}
...
function nowOf(deps: ApplyExecutorDeps): number {
  return deps.now === undefined ? Date.now() : deps.now();
}

/** 生成确定性 applyId(proposal.id + 单次 approval nonce)。 */
export function makeApplyId(proposalId: string, nonce: string): string {
  return `${proposalId}::${nonce}`;
}

function verifyRecordOf(proposal: ApplyProposal, appliedAt: number, now: number): VerifyRecord {
  return {
...
export async function approveAndApply(
  ...
  if (existing !== undefined) {
    ...
    if (existing.status === "applied") return { record: existing, already: true };
  ...
  let written: { value: number; revision: number };
  try {
    ...
    if (code === APPLY_ERROR_CODES.CONFIG_CHANGED) proposal.status = "conflicted";
    await store.proposalTable.put(proposal.id, proposal);
    await store.recordTable.put(input.applyId, record);
...
export async function reconcilePendingApplies(deps: ApplyExecutorDeps): Promise<{ recovered: number }> {
  ...
  for (const [, raw] of entries) {
    const rec = raw as ApplyRecord;
    if (rec.status !== "prepared" && rec.status !== "mutating") continue;
    const current = adapter.readShellTimeout();
...
/** 提案拒绝(只读面,无 mutation)。 */
export async function rejectProposal(deps: ApplyExecutorDeps, proposalId: string): Promise<ApplyProposal> {
  const { store, audit } = deps;
  const proposal = store.proposalTable.get(proposalId) as ApplyProposal | undefined;
  if (proposal === undefined) throw new ApplyError(APPLY_ERROR_CODES.INVALID_PROPOSAL, "proposal not found");

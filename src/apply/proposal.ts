...
import type { ApplyProposal, VerificationPlan } from "./types.js";
...
export function buildShellTimeoutProposal(input: {
  ...
  if (improvement.category !== "TOOL") return null;
  const tool = improvement.evidence.affectedTools[0];
  if (tool === undefined || !SHELL_TOOLS.has(tool)) return null;
  if (improvement.id !== `improve-tool-${tool.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`) return null;
  ...
  if (timeoutCount < SHELL_TIMEOUT_MIN_EVENTS) return null;
  if (timeoutSessions.length < SHELL_TIMEOUT_MIN_SESSIONS) return null;
  // settings 可读 + before 合法。
  if (settings === null) return null;
  const before = settings.value;
  if (!Number.isFinite(before) || before <= 0) return null;
  if (before >= DEEPTRACE_SAFETY_CAP_MS) return null;
  const runtimeCap = Math.min(settings.maxTimeoutMs, DEEPTRACE_SAFETY_CAP_MS);
  const after = Math.min(before * 2, runtimeCap);
  if (!(after > before)) return null;
  if (!(after <= runtimeCap)) return null;

...
function heroHtml(stats: ReportStats, record: ReportRecord, cost?: CostBreakdown): string {
  const totalTokens =
    stats.tokens.input + stats.tokens.output + stats.tokens.cacheRead + stats.tokens.reasoning;
  const items = [
  ...
  if (cost !== undefined) items.push(["预估费用", `¥${cost.total.toFixed(2)}`]);
  return `
  <div class="hero">
...
function gridHtml(stats: ReportStats): string {
  ...
  const cell = (count: number): string => {
    if (count === 0) return '<i style="background:#f1f3f9"></i>';
  ...
  <div class="card"><h2>重试诊断</h2>
    <ul class="samples">
      ${bursts
        ...
        .map(
          (b) =>
            `<li>${esc(b.cmd)}<span>重复 ${b.count} 次 · ${esc(new Date(b.time).toISOString().slice(0, 16).replace("T", " "))}${b.error !== undefined ? ` · ${esc(b.error.slice(0, 80))}` : ""}</span></li>`,
        )
        .join("")}
    </ul>
...
function modelTableHtml(stats: ReportStats, cost?: CostBreakdown): string {
  ...
  if (entries.length === 0) return `<p>无模型用量数据。</p>`;
  return `
  <table class="models">
...
function dangerHtml(stats: ReportStats): string {
  ...
  if (danger.length === 0) return `<p>无危险操作。</p>`;
...
function tokenBarHtml(stats: ReportStats): string {
  ...
  if (total === 0) return "";
  const seg = (value: number, color: string, name: string): string =>
    `<i title="${name} ${fmt(value)}" style="width:${((value / total) * 100).toFixed(2)}%;background:${color}"></i>`;
  return `

...
import { isPeakCstHour, modelTier, PEAK_PRICES, OFFPEAK_PRICES, type CostBreakdown } from "../src/pricing.js";
...
describe("Asia/Shanghai 时间口径矩阵（TZ 无关）", () => {
  ...
  it("hourHistogram 固定（CST 小时桶）", () => {
    const r = computeAll();
    // T 之前的 3 个时间点（23:00 / 03:00 / 10:00 CST），每个 2 个事件（turn + assistant）
    expect(r.hourHistogram[23]).toBe(2);
    expect(r.hourHistogram[3]).toBe(2);
    expect(r.hourHistogram[10]).toBe(2);
  ...
  it("nightRatio 固定（0-6 点 CST 占比）", () => {
    const r = computeAll();
    // 6 个事件中 2 个在 03:00 CST（0-6 点）→ 33.33%
    expect(Math.abs(r.night - 100 / 3)).toBeLessThan(1);
  });

  it("cost 固定（峰谷按 CST 时段）", () => {
    const r = computeAll();
    // 10:00 与 15:00 CST 是 peak（2 倍价）；23:00/03:00 offpeak
    // T 之前 3 个时间点：10:00 CST peak；23:00/03:00 offpeak（15:00 在窗口外）
    const expectCost =
      (300_000 / 1e6) * PEAK_PRICES.flash.inputPerMillion + (1000 / 1e6) * PEAK_PRICES.flash.outputPerMillion +
      (100_000 / 1e6) * OFFPEAK_PRICES.flash.inputPerMillion + (1000 / 1e6) * OFFPEAK_PRICES.flash.outputPerMillion +
      (200_000 / 1e6) * OFFPEAK_PRICES.flash.inputPerMillion + (1000 / 1e6) * OFFPEAK_PRICES.flash.outputPerMillion;
